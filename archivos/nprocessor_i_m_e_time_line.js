// JavaScript Document Update 190112
var swap;
...
function do_load_event(){
	$.ajax({
                url:
                "/ProgrammeProcessorServer/EventProcessorServlet",
                data: "operation=9",
                type: "POST",
                cache: true,
                success: function(response){
                    //Obteniendo elemento de la BD
                    console.log("response: " + response);
                    var descriptor_ = jQuery.parseJSON(response);
                    console.log(descriptor_.lc.pixels);
                    elements_lc=descriptor_.lc;
  					elements_lt=descriptor_.lt;
    				elements_li=descriptor_.li;
    				if(elements_lc.elements.length>0){
    					graphicUpdateElementsSinceIndex(document.getElementById("line_content"), elements_lc, 0);
    				}
